...
class player(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height): #Initialises all the paremeters for the object
        pygame.sprite.Sprite.__init__(self)  
        self.x = x            #This is the x coordinate of the player
        self.y = y            #This is the y coordinate of the player
        self.width = width    #This is the width of the player
        self.height = height  #This is the height of the player
        self.vel = 15         #This is the speed of the player
        self.isJump = False   #This is a varaible which can be reassigned later so that the player can jump
        self.jumpcount = 7    #This is how high th player jumps
        self.left = False     #This determines the left direction of the player 
        self.right =- False   #This determines the right direction of the player 
        self.walkcount = 0    #This counts how far the player has moved
        
    def draw(self,win):
        if self.walkcount + 1 >= 18: #This is a selection statement to see if the player is moving 
            self.walkcount = 0  

        if self.left:
            win.blit(walkLeft[self.walkcount//3], (self.x,self.y))  #This allows the player to move left
            self.walkcount += 1
        elif self.right:
            win.blit(walkRight[self.walkcount//3], (self.x,self.y)) #This allows the player to move left
            self.walkcount += 1
        else:
            win.blit(char, (self.x,self.y)) #This is the standing sprite which is displayed when there is no movement
...
def game_intro():
    ...
    while intro:
        ...
        win.blit(background, (0,0))    #This displays the background for the main menu
        win.blit(castleDash, (465,5))  #This displays the title of the game
        ...
        #About Button
        if 700+103 > mouse[0] > 700 and 117+55 > mouse[1] > 117: #This is the area of the button
            win.blit(aboutButton2, (700,117,103,55))             #Button is highlighted when in this area
            while click[0] == 1:                                                 
                win.blit(aboutmenu, (0,0))                       #When clicked the game_intro function stops and the about screen is played               
            ...
            win.blit(aboutButton, (700,117,103,55)) #This is the default image of the button 




        #Quit Button
        if 700+103 > mouse[0] > 700 and 167+55 > mouse[1] > 167:  #This is the area of the button
            win.blit(quitButton2, (700,167,103,55))               #Button is highlighted when in this area
            if click[0] == 1:
                pygame.quit() #This should quit the game when clicked
                quit() 
        else:
            win.blit(quitButton, (700,167,103,55)) #This is the default image of the button
        ...
        #Feedback Button
        if 695+100 > mouse[0] > 695 and 454+55 > mouse[1] > 454: #This is the area of the button
            win.blit(feedback2, (695,454,100,55))                #Button is highlighted when in this area
            if click[0] == 1:
                url = "https://forms.gle/JmGZC8kPUVRqKM1a7"; 
                webbrowser.open(url, True); #This button will open a page from google with a feedback response sheet
        else:
        ...
        if 500+180 > mouse[0] > 500 and 450+55 > mouse[1] > 450: #This is the area of the button
            win.blit(previous2, (500,450,180,55))                #Button is highlighted when in this area
            if click[0] == 1:
                url = "https://drive.google.com/file/d/1x43TORUTg9qqDRm0sPgzgCSQXnenrMCe/view?usp=sharing"; 
                webbrowser.open(url, True); #This button will open a page from google with a file containing the other versions
        else:
...
#Defining variables before main loop stars 
man = player(300, 357, 64, 64) #Creates the object man which takes the attributes of the class 'player'
run = True                     #When this variable is assigned false it moves onto the next loop
start_time = time.time()       #Starts a timer for the score


#Sounds import 
deathSound = pygame.mixer.Sound(os.path.join('Music', 'dead.wav')) #Imports death sound effect
jumpSound = pygame.mixer.Sound(os.path.join('Music', 'jumps.wav')) #Imports jump sound effect



#Background Music
music = pygame.mixer.music.load(os.path.join('Music', 'music1.mp3')) #Imports music
pygame.mixer.music.play(-1) #Plays the background music on loop 
...
while run:
    ...
    if swords_starty > display_height: #When the sword goes of the screen the sword will start from the beginning
        ...
        swords_startx = random.randrange(display_width)
    ...
    if sword_starty > display_height:
        ...
        sword_startx = random.randrange(display_width)
    ...
    if sword1_starty > display_height:
        ...
        sword1_startx = random.randrange(display_width)    
